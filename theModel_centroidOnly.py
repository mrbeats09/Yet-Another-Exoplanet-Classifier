...
before a regularised classification head.
  - Global branch: centroid channels only (centr1, centr2) × 1000 phase bins.
    Learns coarse features: centroid displacement patterns during transit.
...
OUTPUT_DIR   = "results_centroid_only"
NUM_BINS     = 1000
...
def load_and_preprocess(csv_path):
    ...
    # Stack to (N, 1000, 2) — centroid channels only (RA=ch0, Dec=ch1), channels last
    X = np.stack([centr1, centr2], axis=2)
...
class AugmentedSequence(Sequence):
    ...
    def __getitem__(self, idx):
        ...
        for i in range(len(X_batch)):
            ...
            # 2. Gaussian noise on channel 0 (centroid RA).
            #    In channels-last format, channel 0 is accessed as [:, 0].
            #    Small perturbation on centroid RA channel — applied identically across all model variants for scientific fairness.
            X_batch[i, :, 0] += np.random.normal(0, 0.02, NUM_BINS).astype(np.float32)

            # 3. Scaling on channel 0 (centroid RA): multiply by a factor drawn from [0.95, 1.05].
            #    Small scaling on centroid RA channel — applied identically across all model variants for scientific fairness.
...
def build_model():
    ...
    # ── Global branch: full orbit, centroid channels only (RA=ch0, Dec=ch1) ──
    global_input = keras.Input(shape=(1000, 2), name="global_input")
...
def save_metrics_report(fold_metrics, ensemble_metrics, n_total, output_path):
    ...
    lines = [
        ...
        f"Architecture  : Dual-Branch 1D CNN — Centroid Only Ablation (channels-last)",
...
def main():
    ...
    save_metrics_report(fold_metrics, ensemble_metrics, n_total,
                        os.path.join(OUTPUT_DIR, "metrics_report_centroidOnly.txt"))
