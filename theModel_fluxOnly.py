...
before a regularised classification head.
  - Global branch: flux channel only × 1000 phase bins.
    Learns coarse features: secondary eclipses, OOT variability, flux morphology.
...
def load_and_preprocess(csv_path):
    ...
    # Stack to (N, 1000, 1) — flux channel only, channels last, as TF Conv1D expects
    X = np.stack([flux], axis=2)
...
def build_model():
    ...
    # ── Global branch: full orbit, flux channel only ────────────────────────────
...
def save_metrics_report(fold_metrics, ensemble_metrics, n_total, output_path):
    ...
    lines = [
        ...
        f"Architecture  : Dual-Branch 1D CNN — Flux Only Ablation (channels-last)",
...
def main():
    ...
    save_metrics_report(fold_metrics, ensemble_metrics, n_total,
                        os.path.join(OUTPUT_DIR, "metrics_report_noCentroidInfo.txt"))
